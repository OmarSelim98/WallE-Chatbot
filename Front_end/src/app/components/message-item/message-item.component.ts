...
export class MessageItemComponent implements OnInit {
  ...
  msgBtn:string;
  replyArr = [];
  movieInclude:boolean = false;
  searchInclude:boolean = false;
  movies:any = [];
  ...
  ngOnInit() {
    ...
    if(this.input.message.includes("|||")){
      let result = this.input.message.split("|||");
      if(result[0].trim() == "search"){
        let movies = result[1].trim();
        this.searchInclude = true;
        if(JSON.parse(movies)[0].title){
          console.log(JSON.parse(movies));
          this.movies = JSON.parse(movies);
        }
      }else{
      let movies = result[0].trim();
      let btns = result[1].trim();
      console.log(movies);
      try {
        if(JSON.parse(movies)[0].title){
          console.log(JSON.parse(movies));
          this.movieInclude = true;
          this.movies = JSON.parse(movies);
        }
        this.replyArr = btns.split(/(<button)/);
